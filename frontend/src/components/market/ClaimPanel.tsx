    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
      ...
      <p className="text-xs text-zinc-500 mb-4">
        ...
        <span className="text-white font-medium">
          {hasWinnings ? parseFloat(formatUnits(balance, 18)).toFixed(2) : "0"}
        </span>
      </p>
      <button
        onClick={claim}
        disabled={!hasWinnings || isLoading}
      ...
      >
        {isLoading ? "Claiming..." : hasWinnings ? "Claim Winnings" : "No Winnings to Claim"}
      </button>
    </div>
  );
}
