...
export function useGaslessSpeedBuy() {
  const [isProcessing, setIsProcessing] = useState(false);
  const [txHash, setTxHash] = useState<`0x${string}` | undefined>();

  const executeBuy = async (marketId: bigint, isUp: boolean, amount: string) => {
    ...
    try {
      toast.info("Executing speed trade...");
      ...
      const permit = await autoSignPermit(
        walletClient, account, USDT_ADDRESS, wallet.address, SPEED_MARKET_ADDRESS, parsed,
      );
      ...
      const forwardRequest = await autoSignForwardRequest(
        walletClient, account, wallet.address, SPEED_MARKET_ADDRESS, buyCalldata, 800_000n,
      );
...
export function useGaslessSpeedClaim() {
  ...
  const claim = async (marketId: bigint) => {
    ...
    try {
      toast.info("Claiming speed market winnings...");
      const walletClient = getEmbeddedWalletClient(wallet);
      const account = getEmbeddedAccount(wallet);
      ...
      const forwardRequest = await autoSignForwardRequest(
        walletClient, account, wallet.address, SPEED_MARKET_ADDRESS, claimCalldata, 300_000n,
      );
